...
export const octokit = new MyOctokit({
  ...
  throttle: {
    onRateLimit: (retryAfter, options, octokit, retryCount) => {
      console.log(
        `Request quota exhausted for request ${options.method} ${options.url}. Retrying in ${retryAfter} seconds. Retry count: ${retryCount}`,
      );
...
export async function getCurrentRateLimitRemaining() {
  try {
    ...
    console.log(
      `Rate limit remaining: ${response.resources.core.remaining} out of ${response.resources.core.limit}, reset at ${new Date(response.resources.core.reset * 1000).toLocaleString()}`,
    );
  } catch (error) {
