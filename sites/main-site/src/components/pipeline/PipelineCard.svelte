<script lang="ts">
	...
	if (released) {
		...
		recentRelease =
			...
			add(new Date().getTime(), { days: -1 }).getTime();
	}
</script>
...
<ListingCard {recentRelease}>
	...
	<div slot="card-header">
		<div class="d-flex flex-wrap justify-content-between align-items-center">
			<a
				...
				href={'/' + pipeline.name + '/' + (released ? tagName : 'dev') + '/'}
				>{name}
				{#if archived}
					<i
						class="fa-solid fa-xs fa-archive text-info"
						title="archived"
						data-bs-toggle="tooltip"
					></i>
				{:else if released}
					<i
						class="fa-solid fa-xs fa-check text-success"
						title="released"
						data-bs-toggle="tooltip"
					></i>
				{:else}
	...
	<div slot="card-body" class="d-flex flex-column justify-content-between h-100">
		...
		{#if body}
			<div class="description flex-grow-1" class:pt-1={recentRelease}>
				<Markdown md={body} />
			</div>
		{/if}
		<p class="topics mb-3">
