...
<HackathonGroup title="nf-test plugins" color="#238636" leaders={{'nvnieuwk': 'Nicolas Vannieuwkerke'}}>
...
<HackathonGroup title="nf-core/seqinspector" color="#238636" leaders={{'FranBonath': 'Franziska Bonath'}}>
...
<HackathonGroup title="Single cell analysis: [nf-core/scrnaseq](https://nf-co.re/scrnaseq) and [nf-core/scdownstream](https://nf-co.re/scdownstream)" color="#238636" leaders={{'nictru': 'Nico Trummer'}}>
    nf-core/scrnaseq transforms FASTQ files into expression matrices, while nf-core/scdownstream receives expression matrices as input and performs quality control, integration, clustering and more.
    This project has two main parts:
        1) Move some local modules from scdownstream to nf-core/modules so that they can be re-used in scrnaseq. Basically the preprocessing of scdownstream should be the postprocessing of scrnaseq, so that users can either use scrnaseq and perform their own downstream analysis or they perform their own quantification and use scdownstream afterwards.
        Here you can:
            1) Write tests for existing but local modules and add them to nf-core/modules (beginner friendly)
            2) Integrate the newly added modules into scrnaseq (beginner - intermediate level)
        2) Add new features to scdownstream. See the open [enhancement issues](https://github.com/nf-core/scdownstream/issues?q=is%3Aopen+is%3Aissue+label%3Aenhancement) for details. The base structure of the pipeline is already pretty solid, but there are still some alternative tools that should be supported before the 1.0 release.
        Here you can:
            1) Write new modules and add them to the pipeline (beginner - intermediate level)
            2) Improve the MultiQC report (beginner - intermediate level)
            3) Improve documentation (beginner friendly, prior experience with single cell analyses would be helpful)
            4) Look into a potential extension of the pipeline to multi-omics analyses (advanced level, has not yet been tackled but help would be great!)
