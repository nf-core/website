...
<HackathonGroup title="nf-core/seqinspector" color="#238636" leaders={{'FranBonath': 'Franziska Bonath'}}>
    nf-core/seqinspector aims to become a pipeline, specific for quality control of sequence data. Input is either fastq files or a run folder, and output is one global MultiQC report and, if wished, MultiQC files of groups that are defined in the sample sheet.
