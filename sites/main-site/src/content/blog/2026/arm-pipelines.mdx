...
For developers, there's another compelling reason: Apple Silicon. With ARM support in Bioconda, you can now run your pipelines natively on Mac M1, M2, M3, and M4 machines without emulation overhead. As we discovered during this project, Docker on Mac runs linux/arm64 containers natively (since Docker runs in a Linux VM), so ARM containers are actually faster than emulated x86 ones on Apple Silicon.
...
Angel Pizarro has created a [CloudFormation template](https://gist.github.com/angel-pizarro) that sets up an EC2 Graviton instance with VSCode server, Miniconda, Docker, Nextflow, and nf-core pre-installed. This makes it easy to spin up an ARM development environment in about 10-15 minutes.
...
- **David Lecomber (Arm)** for his tireless work on package porting - submitting hundreds of PRs and creating comprehensive documentation
