...
To ensure maximum compatibility across different environments, nf-core modules are typically set up with a conda `environment.yml`, listing software package dependencies from [Bioconda](https://bioconda.github.io/), see an example [here](https://github.com/nf-core/modules/blob/master/modules/nf-core/fastqc/environment.yml).
...
This means that the nf-core module developer can find the matching [BioContainer](https://biocontainers.pro/) images for their Bioconda package, and then add the image URLs into the module's `main.nf` script, see [here](https://github.com/nf-core/modules/blob/b6b54f3929b0ba4a7c02a49191308ce1d8351f0d/modules/nf-core/fastqc/main.nf#L6-L8).
...
The adoption of Seqera Containers in nf-core was initially raised in the nf-core steering group, followed by discussion in the
...
Note that images from Seqera Containers can already be used in nf-core/modules, with the old syntax of using a `container` declaration, see an example [here](https://github.com/nf-core/modules/blob/b6b54f3929b0ba4a7c02a49191308ce1d8351f0d/modules/nf-core/bedtools/genomecov/main.nf#L6-L8).
