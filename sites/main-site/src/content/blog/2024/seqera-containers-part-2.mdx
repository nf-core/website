...
will need the pipeline developer to hardcode container names and conda lockfiles manually.
...
Failing that, custom containers should be stored under the nf-core account on the quay.io Docker registry.
Only if there are licensing issues should containers at 3rd party registry accounts be used.
