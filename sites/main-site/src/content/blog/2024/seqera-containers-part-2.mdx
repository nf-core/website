...
For the latest discussion and progress on bulk-updating existing nf-core modules, please see GitHub issue
[nf-core/modules#6698](https://github.com/nf-core/modules/issues/6698).
:::

### `main.nf`

We simplify the `container` declaration, listing only the default container image: Docker, for linux/arch64.
There will not be any string interpolation or logic within the container string.
...
Each container / conda environment will have a config file auto-generated by the nf-core/tools CLI.
...
process selectors for each process in the pipeline along with the default docker linux/arch64 container
...
nf-core/tools automation will search the `meta.yml` files for an exact match on the docker linux/arch64 container name
...
:::note{.fa-code title="nextflow.config" collapse}
...
:::tip{.fa-brands.fa-github title="GitHub issue"}
For the latest discussion and updates on this work, please see GitHub issue
