...
pubDate: 2025-08-20T12:00:00+01:00
...
One of Nextflow's strengths is its portability: the same workflow can run in different environments—whether locally, on a high-performance computing cluster, or public cloud platforms. [Google Cloud Batch](https://nextflow.io/docs/latest/executor.html#google-cloud-batch) is the typical way to execute workflows on Google's infrastructure, but Google Colab constitutes an interesting alternative. Due to its generous free allowance for basic computational infrastructure, it is an interesting option for people with limited resources to run computational workflows.
Furthermore, for a fee, you can access the latest hardware each year at a fraction of the cost of buying a new PC.
...
In my experience, this doesn't seem to perform any differently from a shell-based conda installation.
...
I have seen that if you make sure to set up the other aspects of your Colab environment before starting up the tunnel - disconnections rarely happens (or at least drastically reduces the number of).
...
When I was exploring the use of Google Colab for my own work, I encountered specific issues with some pipelines that used tools that had Matplotlib as a dependency.
