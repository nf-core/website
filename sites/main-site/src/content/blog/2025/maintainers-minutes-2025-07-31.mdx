...
As a first step in overhauling this experience we saw in the last major nf-core/tools release the addition of a new sub-command by <Profile username="JulianFlesch">Julian Flesch</Profile> to help explore the available data files in the nf-core/test-datasets repository.
...
Therefore this month's meeting was 'taken over' by the wg-test-dataset-task-force [TODO: add slack channel link] leads Simon <Profile username="SPPearce">Simon</Profile> and <Profile username="jfy133">James</Profile> to start the process of redesigning the structure and documentation.
...
- HuggingFace
    - ✅ Suggested by <Profile username="edmundmiller">Edmund</Profile> as a similar interface to GitHub (thus would be familiar)
...
- AWS S3:
    ...
    - ✅ <Profile username="atrigila">Anabella</Profile> showed services such as 42basepairs [TODO: link] that allow you to see inside common bioinformatics file formats
...
<Profile username="SPPearce">Simon</Profile> also proposed having modules and pipeline test-data in separate locations
...
For example, if we were to follow an object storage concept, it could be that we go with 'chaos' with no directory structure' and everything is organised and guided via the metadata with an user interface layer (as previously proposed by <Profile username="maxulysse">Maxime</Profile>).

## Additional considerations

Other points that were brought up such as:

- We should try to somehow 'version' test-datafiles - e.g. using GitHub URLs pointing to a specific hash, to reduce risks of test breaking if someone changes the contents of a test file (although this shouldn't happen) (Jon <Profile username="pinin4fjords">Jon</Profile>)
- We could maybe consider a 'spill over' location, in case we stick with GitHub and the 10 MB limits are too restrictive for some tools's test-data (which would reduce costs) <Profile username="LouisLeNezet">Louis</Profile>)
- Is there a way to automatically identify data files that have never been used, so we can clean them to save costs (<Profile username="famosab">Famke</Profile>)
