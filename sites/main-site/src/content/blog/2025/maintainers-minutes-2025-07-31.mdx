...
Therefore this month's meeting was 'taken over' by the wg-test-dataset-task-force [TODO: add slack channel link] leads Simon and James [TODO: GitHub Handles] to start the process of redesigning the structure and documentation.
...
- Bioinformatics specific metadata
    ...
    - Genome version
    - Panel
- Support 'grouped' files (e.g. in bioinformatics pairs-end reads, ped/bim/fam, bam/bai)

We then though about different ideas how to store such metadata:

- Stricter and descriptive naming file schema a table listing each
...
Finally, we briefly touched on the structure the repository.
...
Simon [TODO: github handle] also proposed having modules and pipeline test-data in separate locations to make it easier to find the right files and reduce the size.
...
For example, if we were to follow an object storage concept, it could be that we go with 'chaos' with no directory structure' and everything is organised and guided via the metadata with an user interface layer (as previously proposed by Maxime [TODO: github handle]).

## Additional considerations

Other points that were brought up such as:

- We should try to somehow 'version' test-datafiles - e.g. using GitHub URLs pointing to a specific hash, to reduce risks of test breaking if someone changes the contents of a test file (although this shouldn't happen) (Jon [TODO: GITHUB HANDLE])
- We could maybe consider a 'spill over' location, in case we stick with GitHub and the 10 MB limits are too restrictive for some tools's test-data (which would reduce costs) (Louis [TODO: GITHUB HANDLE])
- Is there a way to automatically identify data files that have never been used, so we can clean them to save costs (Famke [TODO: GITHUB HANDLE])
...
- Should we allow 'copying' of a tool's own test-data files or always make our own derived from our existing files (where possible) (Igor: [TODO: GITHUB HANDLE])

And of course, we agreed all the decisions above should be converted into nf-core/proposal RFCs [TODO: link] to facilitate wider community discussions (these will be announced on GitHub when posted!).
...
But we will need a large amount of input from the wider community to ensure the community gets the best experience they want, so we encourage anyone with thoughts and feedback on the above to join the #wg-test-data-task-force [TODO: add link to channel] channel and post their ideas there!
