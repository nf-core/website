...
## `conf/arm.config` → `conf/emulate_amd64.config`

The generic `arm` profile has been replaced with a more specific `emulate_amd64` profile for better ARM64 architecture handling.

#### Resolution

If you had custom changes in your `arm` profile, maintain them when migrating to the new `emulate_amd64` profile configuration.
...
We have updated several key dependencies and configurations:

- Minimum Nextflow version bumped to 25.04.0
- nf-schema updated to 2.5.0
- MultiQC updated to 1.31

#### Resolution

Always accept changes from the template for `nextflow.config` as a baseline, then re-apply your custom modifications.
Make sure to keep any additional plugins you previously added to the configuration.

## `.nf-core.yml`

There might be conflicts in the `repository_type: pipeline` or `template:` sections.

#### Resolution

Accept all changes coming from the template to ensure your pipeline stays up-to-date with the latest template structure.
...
If you were using Gitpod, consider migrating to the new devcontainer setup (`.devcontainer/` directory).

## `subworkflows/local/utils_nfcore_*_pipeline/main.nf`
