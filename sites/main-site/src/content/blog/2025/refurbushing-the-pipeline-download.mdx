...
Each `nf-core` pipeline repository has a GitHub workflow that runs the `nf-core pipelines download` on the pipeline before release (see for example the [`nf-core/rnaseq` workflow](https://github.com/nf-core/rnaseq/actions/workflows/download_pipeline.yml)).
...
In time, there will be a template update to require that `nf-core` pipelines both run the test using the `main` branch of `nf-core/tools` and has this as minimal version of Nextflow.
...
### API change `--parallel-download <int>` becomes `--parallel <int>`

:::warning{title="`API change: --parallel-download <int>` becomes `--parallel <int>`"}
