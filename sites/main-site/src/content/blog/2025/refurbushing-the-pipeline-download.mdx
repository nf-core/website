...
However, as of [Nextflow 25.04](https://www.nextflow.io/docs/latest/migrations/25-04.html) the `nextflow inspect` command has been substantially extended to capture all container strings of modules used in the pipeline.
As of the downloads refactor the `nextflow inspect` command has now also been integrated in the `nf-core/tools` code base to replace the complex regex logic in the downloads commands.
