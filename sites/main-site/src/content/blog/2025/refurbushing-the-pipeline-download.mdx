...
The major problem of the downloads command is to find all containers the pipeline depends on;
it must do this to be able to bundle the software as a standalone package that can be transferred to the offline machine.
Nextflow allows a user to define container both in modules directly or in config files – as long as the container string is resolved at runtime, Nextflow does not care where it came from.
...
Until now, the download command has solved this problem by building up a code base of complex regex patterns used to search through the pipeline’s files for strings resembling container directives.
...
However, as of [Nextflow 25.04](https://www.nextflow.io/docs/latest/migrations/25-04.html) the `nextflow inspect` command has been substantially extended to fetch all pipeline containers by executing a dry-run of the pipeline.
The new `nextflow inspect` command is now integrated in the `nf-core/tools` code base to replace the complex regex logic in the downloads commands.
...
In addition to the use of `nextflow inspect` for container detection,
the download command has now been extended to support Docker containers in addition to Singularity containers which were previously the only container system supported.
This means that `nf-core` pipelines can now be run on offline HPCs which support only Docker or Podman.
...
The saved archives are packaged along with scripts for loading them into Docker or Podman on the offline machine.
