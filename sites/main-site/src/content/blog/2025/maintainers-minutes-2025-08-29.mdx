...
In all cases the various new functionality is not 💯 ready for roll out yet, for example with Harshil Alignment not completely working in the Nextflow former, or our experimental implementations not yet being fully tested, such as with topics being implemented in the nf-core-utils plugin.
...
As always, we want input from the wider community to ensure the community, so we encourage anyone to join the discussion in relevant PRs and Slack channels and threads!
