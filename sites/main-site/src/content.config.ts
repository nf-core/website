  ...
  schema: z
    .object({
      ...
      startTime: z.string().refine((s) => /^(\d{2}:\d{2})([+-]\d{2}:\d{2})$/.test(s), {
        error: "startTime must be in the format HH:MM+|-HH:MM where the +/-HH:MM is the UTC offset",
      }),
      endDate: z.string().refine((s) => /^(\d{4}-\d{2}-\d{2})$/.test(s), {
        error: "endDate must be in the format YYYY-MM-DD",
      }),
      endTime: z.string().refine((s) => /^(\d{2}:\d{2})([+-]\d{2}:\d{2})$/.test(s), {
        error: "endTime must be in the format HH:MM+|-HH:MM where the +/-HH:MM is the UTC offset",
      }),
      announcement: z
        .object({
      ...
      locations: z
        .array(
          z.object({
            ...
            links: z.url().or(z.string().startsWith("#")).or(z.array(z.string().url())).optional(),
