...
1. [ ] Request ownership of the repo to be transferred from the dev
    - If under a organisation repo: ask dev to add the core team member as a co-owner
    - If under a user repo: ask dev to directly transfer ownship to core team member's private account
2. [ ] Transfer the repository to the nf-core organisation
    - Go to repository settings
    - Bottom of General page
    - Press 'Transfer' next to 'Transfer ownership'
    - Keep repository name as is (assuming it's correct based on approved name on proposal)
