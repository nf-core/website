...
These instructions require the person building and pushing the container has push rights to the organisation.
**Only core members may have this access**, and new core members should request access from existing members.

1. Check with the module author that there is really no way to have a conda/bioconda/biocontainer route
...
3. Core team member ensures they have correctly authenticated their docker for pushing to `quay.io`

    ```bash
    docker login --username <QUAY_USER_NAME> quay.io
    ```

4. Core team member builds container with instructions with appropriate tag, e.g.

    ```bash
    docker build . -t quay.io/nf-core/<TOOL>:<VERSION>
    ```

5. Core team member pushes the container

    ```bash
    docker push quay.io/nf-core/<TOOL>:8.0.0
    ```

6. Send instructions to dev to use the new container URL in the module.
    ...
    :::note
    This replaces the entire docker/singularity condition.
    :::
