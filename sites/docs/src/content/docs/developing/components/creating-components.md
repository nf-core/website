...
Shared components enable standardised, reproducible analyses across research groups worldwide, reduce duplicated development effort, and accelerate scientific discovery.
...
1. Run the `nf-core modules list` command:

   ```bash
   nf-core modules list
   ```

1. Check the [nf-core/modules repository](https://github.com/nf-core/modules).

1. Review open [pull requests](https://github.com/nf-core/modules/pulls) and [issues](https://github.com/nf-core/modules/issues).
...
1. Use nf-core/tools to generate the component structure:
   ...
   - **`environment.yml`**: A Conda environment file used by environment and container engines.
   ...
   :::note
   A complete nf-core module may consist of up to two additional files:
   - **`tests/main.nf.test.snap`**: A snapshot file that is generated by nf-test test and used to compare the output of one test run with another.
   - **`tests/nextflow.config`**: A Nextflow configuration file used by nf-test
     :::
...
1. Follow the TODO statements in the generated `main.nf` file.
   ...
   - Ensure adherence to [component specifications](../../specifications/components/overview).

   :::note
   Specifications are designed based on the consensus opinions of the wider nf-core community and span multiple areas. For example:
...
1. Write comprehensive tests in `tests/main.nf.test`.
   ...
   - See the [Testing documentation](../testing/overview) for comprehensive guides on writing assertions and handling complex testing scenarios.
...
- Review the [component specifications](../../specifications/components/overview)
