...
You can pass the module name as an optional argument to `nf-core modules update` instead of using the CLI prompt, for example `nf-core modules update fastqc`. You can specify a pipeline directory other than the current working directory by using the `--dir <pipeline dir>`.
...
If you want this module to be updated only to a specific version (or downgraded), you could instead specify the version:
...
The module versions specified in the `.nf-core.yml` file have higher precedence than versions specified with command-line flags, which helps you write reproducible pipelines.
