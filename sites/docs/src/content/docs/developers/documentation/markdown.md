...
Admonitions are used to highlight text in a box with a title. Use them to draw attention to important information or warn of potential problems.
...
This allows us to add inline code highlighting, line numbers, highlighted lines, file names to code blocks, and more. These directives can be mixed and matched.
...
We support LaTeX based formulas using [KaTeX](https://katex.org/). To display them correctly on an nf-co.re page, wrap them in a block surrounded by `$$`, for example:
...
If you want to get all headings numbered, based on their location and depth in the document, you can set `markdownPlugin: addNumbersToHeadings` in the frontmatter of the markdown file. This will add a number to each heading, for example:
