...
Non-file mandatory arguments required for the module to run without error SHOULD be provided as value channels (for example `lib_type` in [salmon/quant](https://github.com/nf-core/modules/blob/master/modules/nf-core/salmon/quant/main.nf)) - see 'Input/output options' below.
...
As `ext.args` is derived from the configuration (for example, `modules.config`), advanced users can overwrite the default `ext.args` and supply their own arguments to modify the behaviour of a module.
This can increase the capabilities of a pipeline beyond what the original developers intended.

Initially these were passed via the main workflow script using custom functions (for example, `addParams`) and other additional nf-core custom methods, but this syntax overhead and other limitations were more difficult for pipeline developers to use and understand.
...
unless this provides run-time or storage advantages.
...
Multi-tool modules
should chain tools in an explicit order given by the module name. For example, `SAMTOOLS/COLLATEFASTQ`.
:::
...
If a tool does not support compressed input or output natively, we RECOMMEND passing the uncompressed data via unix pipes so that it never gets written to disk, for example:
...
We chose a [HEREDOC](https://tldp.org/LDP/abs/html/here-docs.html) over piping into the versions file line-by-line to avoid accidentally overwriting the file.
...
If the HEREDOC cannot be used because the script is not bash, the `versions.yml` MUST be written directly. For example, [ascat module](https://github.com/nf-core/modules/blob/master/modules/nf-core/ascat/main.nf).
...
The stub block MUST include the same variables (for example, `prefix`) and HEREDOC code as the main script block.

#### Stub files for all output channels

The stub block MUST include the creation of at least one file for every output channel (both mandatory and optional), generated with touch, for example:
...
Stub files output as gzip compressed MUST use the syntax in the following example:
...
:::info{title="Rationale" collapse}
Touching a file with the file name ending in `.gz` will break nf-test's Gzip file parser, as the file is not actually gzipped and thus cannot be read.
...
The directory structure for the module name must be all lowercase, and without punctuation. For example, [`modules/nf-core/bwa/mem/`](https://github.com/nf-core/modules/tree/master/modules/nf-core/bwa/mem/). The name of the software (i.e. `bwa`) and tool (i.e. `mem`) MUST be all one word.

Note that nf-core/tools will validate your suggested name.

### Name format of module processes

The process name in the module file MUST be all uppercase. For example, `process BWA_MEM {`. The name of the software (i.e. `BWA`) and tool (i.e. `MEM`) MUST be all one word separated by an underscore.
...
Any parameters that need to be evaluated in the context of a particular sample. For example, single-end/paired-end data MUST also be defined within the process.
...
- If one tool is multi-threaded and another uses a single thread, you can specify directly in the command itself. For example, with [`${task.cpus}`](https://github.com/nf-core/modules/blob/master/modules/nf-core/bwa/sampe/main.nf#L34)
