...
Use what already exists! The priority is use in your module tests whatever is already available on the `modules` branch of the [nf-core/test-datasets](https://github.com/nf-core/test-datasets) repository. In addition to the `modules` branch, you can look through the other branches to find an appropriate dataset.
