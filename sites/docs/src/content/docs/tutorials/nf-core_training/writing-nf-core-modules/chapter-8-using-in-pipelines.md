...
We assume you have contributed your module to the nf-core/modules repository or a personal/organisation specific module GitHub repository, and it has been merged in and ready for use..

This chapter will explain how to install your module in an nf-core, or your own custom pipeline, and what other wider considerations to take when using them.
...
If you have generated your pipeline skeleton using [nf-core tools](https://nf-co.re/docs/nf-core-tools/pipelines/create) - regardless if this is an official nf-core pipeline or not - you can directly install your module with no setup required!
...
You will also see the `modules.json` for recording the module versions of all nf-core modules you've installed, and also the `.nf-core.yml` configuration file.

The console output will also handily give you a line you can copy and paste into your pipeline script to include the module 💪.

:::warning
If you install a second module, and get an error of `ERROR    'manifest.name`, ensure you have a `nextflow.config` file with the [`manifest` scope](https://www.nextflow.io/docs/latest/reference/config.html#manifest) specified and includes the `name`, `description`, and `version` attributes of the pipeline.
:::
...
Copy and paste the line tools printed to console, into the relevant `.nf` file you want to use the module in.

```nextflow
include { SAMTOOLS_FASTA } from '../modules/nf-core/samtools/fasta/main'
```

You may need to update the path in cases where you are using subworkflow scripts stored in other directories.

Invoke the module, as you would any Nextflow module

```nextflow
SAMTOOLS_FASTA (ch_input_for_samtoolsfasta, val_interleave)
```

However with additional guidances on using specifically modules following nf-core specifications.

### Module channel structures

A important thing to remember when using an nf-core module is to reconfigure your input channels to ensure they match nf-core conventions.

An important component is the use of [meta maps](https://nf-co.re/docs/contributing/components/meta_map).
...
Therefore we highly recommend specifying the `meta.id` attributes in the input channels for all nf-core modules that use meta map.
...
In pipelines using the nf-core template, we already provide with the pipeline template the specifications for each of these labels, stored in the [`conf/base.config` file](https://github.com/nf-core/tools/blob/52e810986e382972ffad0aab28e94f828ffd509b/nf_core/pipeline-template/conf/base.config#L22-L54).
...
If you are using a custom pipeline, you will need to ensure you have a Nextflow config file somewhere in your repository, that is loaded somewhere in your pipeline script (either implicitly such as a `nextflow.config` file or explicitly imported via an `include` as appropriate), that has a process scope with defaults for the label of your installed module specified.
...
This can be done in the same `process` block as the labels in any config file using or loaded in the pipeline.
...
If you want to give a user more freedom if what options and arguments are passed to the tool in the module, you can dynamically pass these to `ext.args` within a closure.
...
Where the `--fatsqc_quiet_mode` is a pipeline level user-facing parameter.

Or even the following, to inject a specific value into the `args` string (which is concatenated with spaces).
...
In summary, using nf-core modules in your pipeline is as simple as installing the module and then invoking it in your pipeline script.
There are a couple of additional considerations to take into account regarding input channels and additional configuration options to specify in Nextflow configuration files, which we have described here.
