...
3. **Core**: Check proposal has received approval in [nf-core/proposals](https://github.com/nf-core/proposals) from either:
...
9. **Core/Dev**: Use `nf-core pipelines create` to create a git repository and push it to GitHub.
10. **Core/Dev**: Once first release (mostly) ready:
    - Transfer the repository to nf-core GitHub organisation and remove the repository on the local machine/user account.
11. **Core/Dev**: Trigger the [GitHub action](https://github.com/nf-core/website/actions/workflows/build-json-files.yml) to sync the `nf-co.re` pipelines page:
