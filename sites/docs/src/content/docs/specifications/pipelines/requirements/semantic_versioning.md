...
- The first release of a pipeline SHOULD be version `1.0.0` (pre-releases are strongly discouraged).
- Release version tags MUST be numerical only (no `v` prefix) and MUST follow [semantic versioning](https://semver.org/) rules: `[major].[minor].[patch]`
