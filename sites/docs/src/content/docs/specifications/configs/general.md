...
Configurations SHOULD comply with, and document, administrative policies of the infrastructure where possible.
...
The subdirectory MUST have the same base name as the config file, for example, for a config called `myinstitute.config`, a directory named `myinstitute`, and it MUST be loaded in the main config with `includeConfig()`.
...
Config names MAY use an underscore.
...
| Parameter                    | Purpose                                                                    |
| ---------------------------- | -------------------------------------------------------------------------- |
| `config_profile_description` | A short description of which infrastructure the config is used for.        |
...
#### Backwards-compatible max parameters

A config MAY also define the `max_*` parameters with the same values as the `resourceLimits` directive.
This provides backward compatibility for older pipelines with older Nextflow versions.
...
Custom config- or infrastructure-specific parameters MUST be documented in the config `.md` file.
