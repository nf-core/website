...
$(function () {
    ...
    $.getJSON('assets/js/fa-icons.json', function (fa_icons) {
        ...
        for (icon in fa_icons) {
            popover_content +=
                '<button class="btn btn-sm btn-light m-1" data-searchterms="' +
                fa_icons[icon] +
                '" data-classname="' +
                icon +
                '"><i class="' +
                        ...
                        for (d in schema['definitions']) {
                            ...
                            if (new_subschema.hasOwnProperty('properties')) {
                                for (k in new_subschema['properties']) {
                                    ...
                                    new_schema['definitions'][new_d]['properties'][new_k] =
                                        new_subschema['properties'][k];
                                }
                            }
                        }
                    }
    ...
    function update_group_params_table() {
        ...
        for (k in schema['properties']) {
            ...
            params +=
                ...
                k +
                `>
                    <td><input type="checkbox" aria-label="Move this parameter" class="select_param" data-id=` +
                k +
                ` id="group-move-` +
                k +
                `"></td>
                    <td><label for="group-move-` +
                k +
...
function generate_param_row(id, param) {
    ...
    if (param.hasOwnProperty('fa_icon') && param['fa_icon'].trim().length > 0) {
        ...
        if (!re.test(param['fa_icon'])) {
            console.error(
                "FontAwesome icon did not match the regex: /^fa[a-z -]+$/ ('" +
                    param['fa_icon'] +
    ...
    var results =
        `
    <div class="row schema_row border" data-id="` +
        id +
        `">
        <div class="col-auto align-self-center schema_row_grabber border-end">
            <i class="fas fa-grip-vertical"></i>
        </div>
        <button class="col-auto align-self-center param_fa_icon">` +
        fa_icon +
        `</button>
        <div class="col schema-id">
            <label>ID
                <input type="text" class="font-monospace param_id" value="` +
        id +
        `">
            </label>
        </div>
        ...
        <div class="col">
            <label>Description
                <input type="text" class="param_key param_description" data-param_key="description" value="` +
        sanitize_html(description) +
        `">
            </label>
        </div>
        <button class="col-auto align-self-center schema_row_help_text_icon" title="Add help text" data-bs-toggle="tooltip">` +
        help_text_icon +
        `</button>
        <div class="col-auto">
            <label>Type
...
function generate_group_row(id, param, child_params) {
    ...
    var results =
        `
    <div class="card schema_group" data-id="` +
        id +
        `">
        <div class="card-header p-0">
            <div class="row schema_row schema_group_row mb-0" data-id="` +
        id +
        `">
                <div class="col-auto align-self-center schema_row_grabber border-end">
                    <i class="fas fa-grip-vertical"></i>
        ...
        `">
                ...
                <div class="col">
                    <label>Description
                        <input type="text" class="param_key" data-param_key="description" value="` +
        sanitize_html(description) +
        `">
                    </label>
                </div>
        ...
        id +
        `">
            <p class="group-drag-drop-help ` +
        drop_help_hidden +
        `">Drag and drop a parameter here</p>
            ` +
        child_params +
        `
        </div>
    ...
    if (param.hasOwnProperty('pattern')) {
        ...
        if (!re.test(param['default'])) {
            alert(
                'Error: Default value "' +
                    param['default'] +
                    '" must match the parameter pattern regex: ' +
                    param['pattern']
            );
            return false;
        }
