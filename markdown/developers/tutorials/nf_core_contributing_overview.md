...
- That all Markdown documentations follow a proper syntax.
  - Many code editors have similar packages to help with markdown validation. For example, check [markdownlint for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=DavidAnson.vscode-markdownlint))
...
Configuration can be done by specifying project-specific default values for the command line options from a pyproject.toml file.

As the file extension suggests, pyproject.toml is a TOML file. It contains separate sections for different tools. Black is using the [tool.black] section.
...
You can also run the software as a package if running it as a script doesn’t work:
