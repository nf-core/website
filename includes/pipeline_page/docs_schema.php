...
if (file_exists($gh_pipeline_schema_fn)) {
  ...
  function print_param($is_group, $param_id, $param, $is_required = false)
  {
    ...
    if (array_key_exists('pattern', $param) && strlen(trim($param['pattern'])) > 0) {
      $pattern_val =
        '<code class="text-small"><span class="text-muted">pattern: </span>' . trim($param['pattern']) . '</code>';
    }
    ...
    if (array_key_exists('enum', $param) && count($param['enum']) > 0) {
      if (count($param['enum']) == 1) {
        ...
        $dropdown_label =
          $default_val != '' ? 'Options: <code class="text-small">' . $param_default . '</code> (default)' : 'Options';
        $enum_val =
          '<div class="btn-group">
    ...
    # Body
    $param_body =
      '<div id="' . $param_id . '-body" class="param-docs-body ' . $description_class . '">' . $description . '</div>';
