...
pubDate: 2024-05-27T00:19:00+01:00
authors:
    - 'nh13'
label:
    - 'pipeline'
...
Developed by [Nils Homer][nilshomer-linkedin-link] at [Fulcrum Genomics][fulcrum-genomics-link], the pipeline utilizes the [`fgbio` Bioinformatics toolkit][fgbio-link] to enable producing ultra-accurate reads for low frequency variant detection.
...
The following [handful of examples from the fgbio wiki][read-structure-examples-link] a attempt to describe the recommended way to describe a sequencing run in two different ways.
...
A few examples from the re shown below:
...
We are working with some of the vendors above to verify that their assays are supported by this pipeline by to obtain test/example data, along with appropriate parameters with which to run this pipeline.
...
<details>
    <summary>Tools to manipulate read-level data:</summary>-   Fastq Manipulation: [`FastqToBam`][fgbio-fastqtobam-link], [`ZipperBams`][fgbio-zipperbams-link], and [`DemuxFastqs`][fgbio-demuxfastqs-link] (see [`fqtk`][fqtk-link], our rust re-implementation for sample demultiplexing).
...
<details>
    <summary>Tools for quality control assessment:</summary>-   Detailed substitution error rate evaluation: [`ErrorRateByReadPosition`][fgbio-errorratebyreadposition-link].
...
<details>
    <summary>Tools for adding or manipulating alternate contig names:</summary>-   Extract from a NCBI Assembly Report: [`CollectAlternateContigNames`][fgbio-collectalternatecontignames-link].
...
<details>
    <summary>Miscellaneous tools:</summary>-   Pick molecular indices (ex. sample barcodes, or molecular indexes): [`PickIlluminaIndices`][fgbio-pickilluminaindices-link] and [`PickLongIndices`][fgbio-picklongindices-link].
